% =============================================================================
% 论文版式集中配置（对照 ../毕业论文格式规范-计算机与信息技术学院.md）
% 正文尽量只写内容与结构，字体字号与页眉页脚等均在本文声明。
% =============================================================================

% ----- 论文元数据（修改封面与个人信息时只改这里） -----
\newcommand{\thesistitleline}{本科生毕业（设计）论文}
\newcommand{\thesissubtitle}{基于生成式AI的知识子图建构与可视化系统的设计与实现}
\newcommand{\thesiscollege}{计算机与信息技术学院}
\newcommand{\thesisgrade}{2026 届}
\newcommand{\thesismajor}{软件工程}
\newcommand{\thesisauthor}{欧阳闻奕}
\newcommand{\thesisstudentid}{2022102069}
\newcommand{\thesisadvisor}{丁蕊}
\newcommand{\thesisenterpriseadvisor}{张杰}
\newcommand{\thesisdateyear}{2026}
\newcommand{\thesisdatemonth}{04}
\newcommand{\thesisdateday}{25}
\newcommand{\thesislogopath}{img/logo.png}
\newcommand{\thesislogowidth}{15cm}

% ----- 宏包 -----
\usepackage{etoolbox}
\usepackage{ctex}
\usepackage{ragged2e}
\usepackage{amsmath,amsfonts,amssymb}
\usepackage{graphicx}
\usepackage[subfigure]{tocloft}
\usepackage{svg}
\usepackage{subfigure}
\usepackage{float}
\usepackage[export]{adjustbox}
\usepackage{listings}
\usepackage{xcolor}
\usepackage{bibentry}
\usepackage[numbers,sort&compress]{natbib}
% 由 \thesisbibliographyheading 单独排“参考文献”标题，禁止 natbib 再 \section*{\refname}
\makeatletter
\renewcommand{\bibsection}{}
\makeatother
\usepackage{abstract}
\usepackage{url}
\usepackage{bm}
\usepackage{multirow}
\usepackage{array}
\usepackage{booktabs}
\usepackage{tabularx}
\usepackage{longtable}
\usepackage{supertabular}
\usepackage{changepage}
\usepackage{enumerate}
\usepackage{caption}
\usepackage{indentfirst}
\usepackage{chngcntr}
\usepackage{titlesec}
\usepackage{fancyhdr}
\usepackage{times}

% 页边距：与 Word 模板主节 A4 设置对齐（上/下 2.54cm，左/右 3.18cm）
\usepackage[left=3.18cm,right=3.18cm,top=2.54cm,bottom=2.54cm,headheight=14pt]{geometry}

\renewcommand{\baselinestretch}{1.5}

% ----- 全局正文字体：中文宋体小四（西文 Times 由 times 包） -----
\AtBeginDocument{\songti\zihao{-4}}

% ----- 摘要环境（中文摘要正文宋体小四；标题黑体小二号居中） -----
\setlength{\absleftindent}{0pt}
\setlength{\absrightindent}{0pt}
\renewcommand{\abstractname}{摘\quad 要}
\renewcommand{\abstractnamefont}{\centering\heiti\zihao{-2}}
\renewcommand{\abstracttextfont}{\normalfont\songti\zihao{-4}}

% ----- 关键词（摘要后空两行再写；关键词标签黑体，内容宋体小四） -----
\newenvironment{thesiskeywords}{%
  \par\vspace{2\baselineskip}%
  \noindent{\heiti\zihao{-4} 关键词：}\songti\zihao{-4}%
}{\par}

% ----- 英文摘要（标题小二号加粗：西文用 \normalfont\bfseries，避免仅设黑体时英文不变粗） -----
\newenvironment{thesisabstracten}{%
  \begin{center}%
  {\zihao{-2}\normalfont\bfseries Abstract\par}%
  \end{center}%
  \vspace{\baselineskip}%
  \begingroup
  \zihao{-4}%
  \setlength{\parindent}{2em}%
}{\par\endgroup}

\newcommand{\thesiskeywordsen}[1]{%
  \par\vspace{0.5\baselineskip}%
  \noindent{\zihao{-4}\normalfont\bfseries Keywords:} \normalfont\zihao{-4}#1\par
}

% ----- 附录：\appendix 后 \thesection 为 A,B,…，但 \arabic{section} 从 1 重计，第\arabic{section}章 会误成“第1章” -----
\newif\ifthesisappendix
\thesisappendixfalse
% 在 article 中于 \appendix 之后将目录中“章”条前缀从「第/章」改为「附录/空」，以得到「附录A」而非「第A章」
\apptocmd\appendix{%
  \thesisappendixtrue
  \addtocontents{toc}{%
    \protect\renewcommand\protect\cftsecpresnum{附录}%
    \protect\renewcommand\protect\cftsecaftersnum{}%
    \protect\setlength\protect\cftsecnumwidth{3.2em}%
  }%
}{}{}

% ----- 章节编号：正文用“第 n 章”；\appendix 后节标题用“附录 A”（与目录一致，不含“章”字，避免 第A章 怪形）-----
\titleformat{\section}[block]{\centering\heiti\zihao{-2}}{%
  \ifthesisappendix
    附录\thesection\quad
  \else
    第\arabic{section}章\quad
  \fi
}{0.5em}{}
\titleformat{\subsection}[hang]{\heiti\zihao{-3}}{\thesubsection}{0.5em}{}
\titleformat{\subsubsection}[hang]{\heiti\zihao{4}}{\thesubsubsection}{0.5em}{}
\titlespacing*{\section}{0pt}{1.5ex plus 0.3ex minus 0.2ex}{0.8ex plus 0.2ex}
\titlespacing*{\subsection}{2em}{1.2ex plus 0.3ex minus 0.2ex}{0.6ex plus 0.2ex}
\titlespacing*{\subsubsection}{2em}{1ex plus 0.3ex minus 0.2ex}{0.5ex plus 0.2ex}

% ----- 图、表按章编号：图 1-1、表 1-1；题注黑体五号 -----
\counterwithin{figure}{section}
\counterwithin{table}{section}
% 正文为「章号-表号」；\appendix 后章号为 A、B，故用 \thesection
\renewcommand{\thefigure}{%
  \ifthesisappendix\thesection\else\arabic{section}\fi-\arabic{figure}}
\renewcommand{\thetable}{%
  \ifthesisappendix\thesection\else\arabic{section}\fi-\arabic{table}}
\renewcommand{\figurename}{图}
\renewcommand{\tablename}{表}
% 图题、表题：黑体五号（10.5pt）
\DeclareCaptionFont{thesiscap}{\heiti\fontsize{10.5pt}{15.75pt}\selectfont}
\captionsetup{
  font=thesiscap,
  labelfont=thesiscap,
  textfont=thesiscap,
  labelsep=space,
}

% ----- 目录标题 -----
\renewcommand{\contentsname}{目录}
% 目录标题：黑体小二号居中（tocloft 双侧 \hfill 夹住标题）
\renewcommand{\cfttoctitlefont}{\hfill\heiti\zihao{-2}}
\renewcommand{\cftaftertoctitle}{\hfill\null\par\vspace{1\baselineskip}}

% ----- 代码 listings -----
\lstdefinelanguage{cypher}{
  morekeywords={MATCH,RETURN,WHERE,AS,WITH,ORDER,LIMIT,OPTIONAL,CREATE,DELETE,MERGE,DETACH,SET,REMOVE,DROP,CALL,UNWIND,UNION,AND,OR,NOT,IN,CONTAINS,STARTS,ENDS,IS,NULL,CASE,WHEN,THEN,ELSE,END,DISTINCT,SKIP},
  sensitive=false,
  morecomment=[l]{//},
  morestring=[b]',
}
\lstdefinelanguage{json}{
  morekeywords={true,false,null},
  sensitive=false,
  morestring=[b]",
  morecomment=[l]{//},
}
% 代码块等宽：Monaspace Radon（fontconfig 中须用 “Regular” 等全名；勿用 “Monaspace Radon” 会解析失败。无字体时改 basicstyle 为 \small\ttfamily 并删除本 \newfontfamily。）
\newfontfamily\ThesisListingsBase{Monaspace Radon}[
  UprightFont     = {Monaspace Radon Regular},
  ItalicFont      = {Monaspace Radon Italic},
  BoldFont        = {Monaspace Radon Bold},
  BoldItalicFont  = {Monaspace Radon Bold Italic},
  Scale = 0.90
]
\lstset{
  basicstyle={\ThesisListingsBase\small},
  breaklines=true,
  frame=single,
  backgroundcolor=\color{gray!10},
  rulecolor=\color{gray!30},
  numbers=none,
  showstringspaces=false,
  tabsize=2,
  aboveskip=10pt,
  belowskip=10pt
}

\setlength{\cftbeforesecskip}{3pt}
\setlength{\cftbeforesubsecskip}{2pt}
\setlength{\cftbeforesubsubsecskip}{1pt}
% 目录中章条显示“第 n 章”（\thesection 仍为纯数字，故用 presnum/aftersnum）
\renewcommand{\cftsecpresnum}{第}
\renewcommand{\cftsecaftersnum}{章}
\setlength{\cftsecnumwidth}{4.2em}
% article + tocloft 默认 \cftsecdotsep=\cftnodots，“章”级目录无引导点；摘要/Abstract/第n章 与正文章条应统一为点线至页码（导师意见第2条）
\renewcommand{\cftsecdotsep}{\cftdotsep}
% tocloft 默认在目录标题处执行 \thispagestyle{plain}，会打出页码；改为与目录专用样式一致
\tocloftpagestyle{thesistoc}

% ----- 页眉页脚 -----
\newcommand{\thesisheaderline}{牡丹江师范学院本科生毕业论文（设计）}
\fancypagestyle{thesisempty}{%
  \fancyhf{}%
  \renewcommand{\headrulewidth}{0pt}%
}
\fancypagestyle{thesisfront}{%
  \fancyhf{}%
  \fancyhead[C]{\kaishu\zihao{5}\thesisheaderline}%
  \fancyfoot[C]{\thepage}%
  \renewcommand{\headrulewidth}{0.4pt}%
}
% 目录页：保留页眉，不显示页码（且目录不写入目录）
\fancypagestyle{thesistoc}{%
  \fancyhf{}%
  \fancyhead[C]{\kaishu\zihao{5}\thesisheaderline}%
  \renewcommand{\headrulewidth}{0.4pt}%
}
\fancypagestyle{thesismain}{%
  \fancyhf{}%
  \fancyhead[C]{\kaishu\zihao{5}\thesisheaderline}%
  \fancyfoot[C]{\thepage}%
  \renewcommand{\headrulewidth}{0.4pt}%
}

% ----- 封面（与 Word 模板：左标签三号黑体，论文题目内容小二号黑体；日期三号居中） -----
\newcommand{\thesiscover}{%
  \thispagestyle{thesisempty}%
  \begin{center}
    \includegraphics[width=\thesislogowidth]{\thesislogopath}%
    \vspace{0.6\baselineskip}
    {\heiti\zihao{-2}\thesistitleline\par}%
    \vspace{1.8cm}%
  \end{center}%
  \noindent
  \renewcommand{\arraystretch}{1}%
  \begin{tabular}{@{}p{4.2cm}@{\hspace{0.6em}}p{\dimexpr\linewidth-4.2cm-0.6em\relax}@{}}%
    {\heiti\zihao{3} 论文题目：} &
      {\heiti\zihao{-2}\thesissubtitle\par} \\
    {\heiti\zihao{3} 学\qquad 院：} &
      {\heiti\zihao{3}\thesiscollege\par} \\
    {\heiti\zihao{3} 年\qquad 级：} &
      {\heiti\zihao{3}\thesisgrade\par} \\
    {\heiti\zihao{3} 专\qquad 业：} &
      {\heiti\zihao{3}\thesismajor\par} \\
    {\heiti\zihao{3} 姓\qquad 名：} &
      {\heiti\zihao{3}\thesisauthor\par} \\
    {\heiti\zihao{3} 学\qquad 号：} &
      {\heiti\zihao{3}\thesisstudentid\par} \\
    {\heiti\zihao{3} 指导教师：} &
      {\heiti\zihao{3}\thesisadvisor\par} \\
    {\heiti\zihao{3} 行企业指导教师：} &
      {\heiti\zihao{3}\ifx\thesisenterpriseadvisor\empty ——\else\thesisenterpriseadvisor\fi\par} \\
  \end{tabular}%
  \par\vfill
  \begin{center}
    {\heiti\zihao{3}\thesisdateyear 年\thesisdatemonth 月\thesisdateday 日\par}%
  \end{center}%
  \clearpage
}

% 后记级标题（结论 / 致谢 / 参考文献）：黑体小二号居中；不用 \section* 以免与 titlesec 冲突
\newcommand{\thesisbackmattertitle}[1]{%
  \begin{center}{\heiti\zihao{-2} #1\par}\end{center}%
  \vspace{\baselineskip}%
}

\newenvironment{thesisconclusion}{%
  \clearpage
  \phantomsection
  \thesisbackmattertitle{结论}%
  \addcontentsline{toc}{section}{结论}%
  \begingroup
  \songti\zihao{-4}%
}{%
  \par\endgroup
}

\newenvironment{thesisacknowledgement}{%
  \clearpage
  \phantomsection
  \thesisbackmattertitle{致谢}%
  \addcontentsline{toc}{section}{致谢}%
  \begingroup
  \songti\zihao{-4}%
}{%
  \par\endgroup
}

\newcommand{\thesisbibliographyheading}{%
  \clearpage
  \phantomsection
  \thesisbackmattertitle{参考文献}%
  \addcontentsline{toc}{section}{参考文献}%
}

% ----- 颜色（按需保留） -----
\newcommand{\red}[1]{\textcolor[rgb]{1.00,0.00,0.00}{#1}}
\newcommand{\blue}[1]{\textcolor[rgb]{0.00,0.00,1.00}{#1}}
\newcommand{\green}[1]{\textcolor[rgb]{0.00,1.00,0.00}{#1}}
\newcommand{\darkblue}[1]{\textcolor[rgb]{0.00,0.00,0.50}{#1}}
\newcommand{\darkgreen}[1]{\textcolor[rgb]{0.00,0.37,0.00}{#1}}
\newcommand{\darkred}[1]{\textcolor[rgb]{0.60,0.00,0.00}{#1}}
\newcommand{\brown}[1]{\textcolor[rgb]{0.50,0.30,0.00}{#1}}
\newcommand{\purple}[1]{\textcolor[rgb]{0.50,0.00,0.50}{#1}}

\usepackage{hyperref}
\hypersetup{colorlinks=true,linkcolor=black}
